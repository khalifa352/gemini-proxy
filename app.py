...
def optimize_path_data(d):
    ...
    if d.strip() and d.strip()[-1].upper() != 'Z' and d.strip().startswith('M'):
        coords = re.findall(r"[-+]?\d*\.?\d+", d)
        if len(coords) >= 4:
            try:
                if abs(float(coords[0]) - float(coords[-2])) < 0.01 and abs(float(coords[1]) - float(coords[-1])) < 0.01:
                    d += 'Z'
            except Exception:
                pass
...
def validate_svg_quality(svg_code):
    ...
    for p in paths:
        if "Z" not in p.upper() and "C" in p.upper():
