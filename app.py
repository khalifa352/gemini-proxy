...
for p in range(pages):
    ...
    if p == 0:
        bg_images += f'<image href="data:image/jpeg;base64,{letterhead_b64}" x="0" y="0" width="{width}" height="{height}" preserveAspectRatio="none" />\n'
...
try:
    ...
    gen_config = types.GenerateContentConfig(
        ...
        temperature=0.2,  # تقليل الإبداع العشوائي
    ...
    # تنظيف الألوان البيضاء (قد تغطي على المحتوى)
    final_svg = re.sub(
...
try:
    ...
    gen_config = types.GenerateContentConfig(
        ...
        temperature=0.15,  # منخفضة للحفاظ على الاستقرار
        max_output_tokens=16384  # زيادة للسماح بصفحات متعددة
    )
